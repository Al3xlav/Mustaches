    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recordingBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Session session = customArFragment.getArSceneView().getSession();
                if (session.getRecordingStatus() == RecordingStatus.NONE) {
                    recordingBtn.setText("Stop recording");
//                    File path = new File(String.valueOf(Environment.getExternalStorageDirectory()), "films");
//                    if (!path.exists()) {
//                        path.mkdir();
//                    }
//                    File file = new File(path, "recording.mp4");
//                    Uri destination = Uri.fromFile(file);
                    Uri destination = Uri.fromFile(new File(String.valueOf(Environment.getExternalStorageDirectory()), "film.mp4"));
